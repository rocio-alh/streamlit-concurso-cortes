...
def simulador_lote(nombre_lote, presupuesto_base, pmax_precio, pmax_garantia, p_tecnico_max):
    with st.expander(f"📦 {nombre_lote} - Simulación de tu puntuación (con competidores)", expanded=False):
        ...
        c1, c2 = st.columns(2)
        with c1:
            oferta = st.number_input(f"💰 Tu precio ofertado (€) - {nombre_lote}",
        ...
        for crit, max_pts in criterios.items():
            puntos_tecnicos += st.slider(f"{crit} (0 - {max_pts})", 0, max_pts, max_pts,
                                         key=f"{nombre_lote}_{crit}")
        ...
        with c1:
            num_empresas = st.number_input("Número total de ofertas (incluyéndote)", min_value=2, value=5, step=1,
                                           key=f"num_{nombre_lote}")
        with c2:
            distribucion = st.selectbox("Distribución de precios simulados",
                                        ["Uniforme", "Normal", "Triangular"], key=f"dist_{nombre_lote}")

        bajada_max = st.slider("Bajada máxima esperada respecto al PL (%)", 1, 50, 20, key=f"bajada_{nombre_lote}")
        min_precio_sim = presupuesto_base * (1 - bajada_max / 100)
        num_comp = num_empresas - 1
        ...
        for nombre, precio in zip(nombres_empresas, precios_finales):
            ...
            g = garantia if nombre == "Tú" else np.random.uniform(u1_garantia, u2_garantia + 1)
            p_g = calcular_puntuacion_garantia(g, u1_garantia, u2_garantia, pmax_garantia)
            p_t = puntos_tecnicos if nombre == "Tú" else np.random.randint(20, p_tecnico_max + 1)
            total = p_p + p_g + p_t
            tabla.append({
                "Empresa": nombre,
        ...
        total_usuario = df.loc[df["Empresa"] == "Tú", "Total"].values[0]
...
empresas = st.session_state.get("empresas_Lote 1", ["Tú", "Empresa 1", "Empresa 2"])
...
lote1_vals = ajustar_longitud(st.session_state.get("puntos_Lote 1", [0.0]*num_empresas), num_empresas)
lote2_vals = ajustar_longitud(st.session_state.get("puntos_Lote 2", [0.0]*num_empresas), num_empresas)
lote3_vals = ajustar_longitud(st.session_state.get("puntos_Lote 3", [0.0]*num_empresas), num_empresas)
...
for emp in empresas:
    with st.expander(f"⚙️ Oferta integradora de {emp}", expanded=False):
        incluye = st.multiselect(f"Lotes incluidos en la oferta integradora de {emp}",
...
if ofertas_integradoras:
    ...
    empresas_base = empresas
    puntuaciones_ind = {emp: {"L1": lote1_vals[i], "L2": lote2_vals[i], "L3": lote3_vals[i]} for i, emp in enumerate(empresas)}
    ...
    for oferta in ofertas_integradoras:
        ...
        lotes_int = oferta["incluye"]
        if set(lotes_int) == {"L1", "L2"}:
    ...
    for c in combinaciones:
        ...
        resultados.append({"Combinación": c["nombre"], "L1": c["L1"], "L2": c["L2"], "L3": c["L3"],
